  ...
  <div class="min-h-screen text-white p-6 bg_app">
    <div class="flex mb-1">
      ...
      <router-link to="/GameVariants" class=" ">
        <Button size="small" class="ml-2"> Шаблоны ролей </Button>
      </router-link>
    </div>
    ...
    <template v-if="savedGame">
      <div>
        ...
        >
          ...
          >
            У вас есть сохраненная игра!
          </h5>
          <p class="font-normal text-gray-700 dark:text-gray-400 mb-2">
            Вы можете продолжить или сбросить и начать новую:
          </p>
          <router-link to="/Game">
    ...
    <template v-else>
      ...
      <div>
        ...
        <div
          v-for="role in rolesItems"
          :key="role.id"
          class="opacity-75 flex gap-2 mb-2"
        >
          <Checkbox
            :value="role.name"
            v-model="roles"
          ...
          <label :for="'role-' + role.id">
            ...
            <span style="font-size: 12px; opacity: 0.35"
              >({{ role.name_en.slice(0, 7) }}..)</span
            >
          </label>
        </div>
      </div>
      ...
      <div class="flex items-center gap-4">
        ...
        <div class="text-sm opacity-50">Ролей: {{ roles.length }}</div>
        <div class="text-sm opacity-50">Игроков: {{ players }}</div>
      </div>
    </template>
    <hr class="my-2 opacity-25" />
